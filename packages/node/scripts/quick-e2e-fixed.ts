...
async function main() {
  ...
  const pr = await api('POST', `${N1}/posts`, { content: 'Hello DAGsocial!', author: aPubH, parentRefs: [], challenge: chal.data.challenge, powNonce: nonce, protocolVersion: PROTOCOL_VERSION, timestamp: ts, signature: hex(po.signature), karmaLockTx: ltx });
  ...
  P(await api('POST', `${N1}/posts`, { content: 'Reply!', author: aPubH, parentRefs: [ppid], challenge: chal2.data.challenge, powNonce: rn, protocolVersion: PROTOCOL_VERSION, timestamp: ts2, signature: hex(ro.signature), karmaLockTx: rltx }), 'create reply');
  await sleep(12000);
  ...
  if (bBid && bBal >= LIKE_COST) {
    ...
    P(await api('POST', `${N1}/likes`, { tx: ltx2 }), 'cast like');
    await sleep(12000);
